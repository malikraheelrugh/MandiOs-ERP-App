    ...
    <div className="space-y-6">
      ...
      <div className="hidden print:block text-slate-900 border-b-2 border-slate-900 pb-4 mb-4">
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-xl font-black uppercase tracking-wider">MANDI OS — SUPPLIER / GROWER ACCOUNT STATEMENT</h1>
            <p className="text-xs font-bold text-indigo-800">
              Supplier: {supplierDetails?.name || 'Grower / Supplier'} {supplierDetails?.phone ? `| Phone: ${supplierDetails.phone}` : ''}
            </p>
          ...
          <div className="text-right border border-slate-400 p-2 rounded bg-slate-50">
            ...
            <p className="text-sm font-black text-slate-900">
              Rs. {Math.abs(currentBalance).toLocaleString()} {currentBalance > 0 ? '(Cr - Payable to Supplier)' : currentBalance < 0 ? '(Dr - Advance to Supplier)' : '(Settled)'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mt-3 pt-2 border-t border-slate-300 text-center text-[10px]">
          <div className="bg-slate-100 p-1.5 rounded">
            <span className="font-bold text-slate-600">Total Payments Disbursed (Debit):</span> <span className="font-black text-slate-900">Rs. {totalDebit.toLocaleString()}</span>
          </div>
          <div className="bg-slate-100 p-1.5 rounded">
            <span className="font-bold text-slate-600">Net Consignment Sales (Credit):</span> <span className="font-black text-slate-900">Rs. {totalCredit.toLocaleString()}</span>
          </div>
          <div className="bg-slate-100 p-1.5 rounded">
